class OptionsController < ApplicationController
  ...
  # GET /options.json
  def index
    @options = current_account.options.order(sort_column + " " + sort_direction).page(params[:page])
  ...
  def new
    @option = current_account.options.new
  ...
  # POST /options.json
  def create
    @option = current_account.options.new(params[:option])
...
private
  ...
  def sort_column
    Option.column_names.include?(params[:sort]) ? params[:sort] : "name"
  end
end