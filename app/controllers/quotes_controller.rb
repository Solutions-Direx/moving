class QuotesController < ApplicationController
  ...
  def monthly
    ...
    @quotes = Quote.where(removal_at: (@date - 1.month).beginning_of_month..(@date + 1.month).end_of_month).order('removal_at ASC')
  end
  
  def daily
    set_tab :calendar
    @day = params[:day] ? Date.strptime(params[:day], "%Y-%m-%d") : Date.today
    @quotes = Quote.where(removal_at: @day.beginning_of_day..@day.end_of_day).order('removal_at ASC')
  end
